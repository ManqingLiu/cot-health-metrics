...
class ChatJsonlDataset(Dataset):
    ...
    def _mask_labels(self, prompt_ids, full_ids, assistant_text: str):
        ...
        if self.mask_mode == "assistant":
            return labels
        asst_ids = self.tok(assistant_text, return_tensors="pt",
    ...
    def __getitem__(self, idx: int):
        ...
        if prompt_ids.shape[1] >= self.max_length:
            # sample cannot fit even the prompt -> force user to raise max_length or shorten data
            raise ValueError(
        ...
        if asst_ids.shape[1] <= room:
            ...
            # want a window of length <= room that ends at the sequence end (to keep Answer), but if possible starts at the <think> tag
...
def load_model_with_fallbacks(model_id: str, cache_dir: Optional[str], torch_dtype=None,
    ...
    """
    initially- Try normal Auto* with trust_remote_code=True; if it fails because arch is unknown to Transformers, try to load via repo auto_map
    """
    import logging
    try:
    ...
    if hf_hub_download is not None:
        ...
        if not auto_map:
            raise RuntimeError(
                "Could not load the model. The Transformers version likely doesn't support this architecture / the repo does not expose remote code via auto_map"
            )
